"""Dehollander 2024 dotcloud risky-choice — per-fit report.

Loads production fits from
/shares/zne.uzh/gdehol/bauer_results/dehollander_dotcloud/30subj/ and produces
diagnostics, group + per-subject posteriors, PPCs (paper Fig 1B/1C style),
and implied σ_k(n) for flex models.
"""
# %%
...
RES = '/shares/zne.uzh/gdehol/bauer_results/dehollander_dotcloud/30subj'
OUT = '/Users/gdehol/git/bauer/notebooks/figures/dehollander_dotcloud_report'
...
for name, idata in idatas.items():
    ...
    ax.set_title(f'Dehollander dotcloud — {name}: group-level parameters')
    plt.tight_layout()
...
for name, idata in idatas.items():
    ...
    g.fig.suptitle(f'Dehollander dotcloud — {name}: per-subject posteriors (95% CI)', y=1.02)
...
for name in ['choice-flex', 'DDM-flex', 'RDM-flex']:
    ...
    ax.set_title(f'Dehollander dotcloud — {name}: implied encoding noise (94% HDI)')
    ax.legend()
    plt.tight_layout()
    fig.savefig(op.join(OUT, f'sigma_{name}.png'), dpi=140, bbox_inches='tight')
    plt.close(fig)

# %% PPC — TODO: paper Fig 1B (P(chose risky) vs n_safe × order)
#               + Fig 1C (psychometric / chronometric vs log_ratio binned × order)
#    Will use sns.lineplot with errorbar=('pi', 80) for PPC bands and
#    sns.scatterplot for data points.
