...
class LapseModel(BaseModel):
    ...
    #     (the original behaviour) under ``lapse_group``.
    #   - a FLOAT: the choice-lapse rate is FIXED at that value; it is NOT a
    #     sampled parameter, so it cannot funnel/diverge. Useful for hard
    #     geometries (e.g. full-prior estimation) where a free hierarchical lapse
    #     trades one funnel for another, and -- as for the RT models' contaminant
    #     -- where you'd rather assume a small lapse than try to estimate it.
    p_lapse = None
